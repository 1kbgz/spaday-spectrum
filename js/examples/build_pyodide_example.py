...
def main(output: Path, spectrum_wheel: Path, dependency_dir: Path) -> None:
    spaday_wheel = one_wheel(dependency_dir, "spaday")
    transports_wheel = one_wheel(dependency_dir, "transports")
    wheels = {
        "spectrum": copy_wheel(spectrum_wheel, output),
        "spaday": copy_wheel(spaday_wheel, output),
        "transports": copy_wheel(transports_wheel, output),
    }
    extract_assets(spectrum_wheel, "spaday_spectrum", output / "components" / "spectrum")
